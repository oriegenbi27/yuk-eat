    ...
    (function(factory) {
      ...
      var DataTable = function(selector, options) {
        ...
        this.each(function() {
          ...
          if ($.inArray(true, $.map(stripeClasses, function(el, i3) {
            ...
            $(rowOne[0]).children("th, td").each(function(i3, cell) {
              ...
              if (col.mData === i3) {
                ...
                if (sort !== null || filter !== null) {
                  col.mData = {
                    _: i3 + ".display",
                    sort: sort !== null ? i3 + ".@data-" + sort : undefined,
                    type: sort !== null ? i3 + ".@data-" + sort : undefined,
                    filter: filter !== null ? i3 + ".@data-" + filter : undefined
                  };
          ...
          if ($.isPlainObject(source)) {
            ...
            var setData = function(data, val, src) {
              ...
              for (var i2 = 0, iLen = a.length - 1; i2 < iLen; i2++) {
                ...
                if (data[a[i2]] === null || data[a[i2]] === undefined) {
                  data[a[i2]] = {};
                }
                data = data[a[i2]];
              }
            ...
            var fetchData = function(data, type, src) {
              ...
              if (src !== "") {
                ...
                for (var i2 = 0, iLen = a.length; i2 < iLen; i2++) {
                  ...
                  if (data === null || data[a[i2]] === undefined) {
                    return undefined;
                  }
                  data = data[a[i2]];
                }
      ...
      function _fnVisbleColumns(oSettings) {
        var vis = 0;
        $.each(oSettings.aoColumns, function(i2, col) {
      ...
      function _fnGetColumns(oSettings, sParam) {
        var a = [];
        $.map(oSettings.aoColumns, function(val, i2) {
          if (val[sParam]) {
            a.push(i2);
          }
        });
      ...
      function _fnDrawHead(oSettings, aoSource, bIncludeHidden) {
        ...
        for (i2 = 0, iLen = aoLocal.length; i2 < iLen; i2++) {
          ...
          for (j = 0, jLen = aoLocal[i2].length; j < jLen; j++) {
            iRowspan = 1;
            iColspan = 1;
            if (aApplied[i2][j] === undefined) {
              nLocalTr.appendChild(aoLocal[i2][j].cell);
              aApplied[i2][j] = 1;
              while (aoLocal[i2 + iRowspan] !== undefined && aoLocal[i2][j].cell == aoLocal[i2 + iRowspan][j].cell) {
                aApplied[i2 + iRowspan][j] = 1;
                iRowspan++;
              }
              while (aoLocal[i2][j + iColspan] !== undefined && aoLocal[i2][j].cell == aoLocal[i2][j + iColspan].cell) {
                for (k = 0; k < iRowspan; k++) {
                  aApplied[i2 + k][j + iColspan] = 1;
                }
                iColspan++;
              }
              $(aoLocal[i2][j].cell).attr("rowspan", iRowspan).attr("colspan", iColspan);
            }
          }
        }
      }
      ...
      function _fnScrollDraw(settings) {
        ...
        $.each(_fnGetUniqueThs(settings, headerCopy), function(i2, el) {
      ...
      function _fnSortFlatten(settings) {
        var i2, iLen, k, kLen, aSort = [], aiOrig = [], aoColumns = settings.aoColumns, aDataSort, iCol, sType, srcCol, fixed = settings.aaSortingFixed, fixedObj = $.isPlainObject(fixed), nestedSort = [], add = function(a) {
        ...
        for (i2 = 0; i2 < nestedSort.length; i2++) {
          ...
          for (k = 0, kLen = aDataSort.length; k < kLen; k++) {
            ...
            if (nestedSort[i2]._idx === undefined) {
              nestedSort[i2]._idx = $.inArray(nestedSort[i2][1], aoColumns[iCol].asSorting);
            }
            aSort.push({
              src: srcCol,
              col: iCol,
      ...
      function _fnSaveState(settings) {
        ...
        var state = {
          ...
          columns: $.map(settings.aoColumns, function(col, i2) {
            return {
        ...
        if (s.columns) {
          for (i2 = 0, ien = s.columns.length; i2 < ien; i2++) {
            ...
            if (col.search !== undefined) {
              $.extend(settings.aoPreSearchCols[i2], _fnSearchToHung(col.search));
            }
          }
          if (api) {
            api.columns.adjust();
      ...
      function _fnCallbackFire(settings, callbackArr, eventName, args) {
        var ret = [];
        if (callbackArr) {
          ret = $.map(settings[callbackArr].slice().reverse(), function(val, i2) {
      ...
      var _toSettings = function(mixed) {
        var idx, jq;
        var settings = DataTable.settings;
        var tables = $.map(settings, function(el, i2) {
          return el.nTable;
        });
        if (!mixed) {
          return [];
      ...
      var __table_selector = function(selector, a) {
        ...
        var nodes = $.map(a, function(el, i2) {
          return el.nTable;
        });
        return $(nodes).filter(selector).map(function(i2) {
        ...
        if (_fnDataSource(settings) == "ssp") {
          ...
          for (i2 = 0, ien = settings.aoData.length; i2 < ien; i2++) {
            if (search == "none") {
              ...
              tmp = $.inArray(i2, displayFiltered);
          ...
          if (match) {
            switch (match[2]) {
              ...
              case "visible":
                ...
                if (idx < 0) {
                  var visColumns = $.map(columns, function(col, i2) {
              ...
              case "name":
                return $.map(names, function(name, i2) {
      ...
      $.each(["on", "one", "off"], function(i2, key) {
          ...
          if (!remove && orig) {
            ...
            if (ien) {
              jqTbody.children().each(function(i3) {
                $(this).addClass(settings.asDestroyStripes[i3 % ien]);
              });
            }
          }
      ...
      function __mldObj(d, format, locale) {
        ...
        if (window2.moment) {
          ...
          dt = format ? window2.luxon.DateTime.fromFormat(d, format) : window2.luxon.DateTime.fromISO(d);
          if (!dt.isValid) {
            return null;
          }
          dt.setLocale(locale);
